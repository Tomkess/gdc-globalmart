...
- [x] 35. Add the CI job running `uv run globalmart normalize --check layouts/workspaces/globalmart` plus `uv run pytest tests/ -x -q` on every PR, so a hand-edited layout file fails the build.
