...
- [ ] 1. Given the committed parent tree and `domains.yaml`, when `globalmart split` runs, then
      ...
      manifest.
- [ ] 2. Given an unchanged parent tree and manifest, when `globalmart split` is run twice, then
...
- [ ] 3. Given the `hr` domain, when its generated file is inspected, then `ldm.datasets` contains
...
- [ ] 4. Given any generated child, when every dataset in its LDM is checked against the union of
...
- [ ] 5. Given any retained dataset — whether it arrived through analytics closure, through a
...
- [ ] 6. Given the parent's 2 date instances, when a child is generated, then `ldm.date_instances`
...
- [ ] 7. Given a retained metric whose MAQL references another metric via `{metric/<id>}`, when the
...
- [ ] 8. Given a domain whose closure references an object that does not exist in the parent (a
...
- [ ] 9. Given the full manifest, when `split` completes, then every one of the parent's 32
...
- [ ] 10. Given a domain's nested AI-context selection in `domains.yaml` (`ai.memory_item_ids`,
...
- [ ] 11. Given the parent's `attributeHierarchies`, `exportDefinitions`, `dashboardPlugins`,
...
- [ ] 12. Given a generated child, when it is searched for datasource identifiers, then every
...
- [ ] 13. Given `globalmart publish domains --target <profile>` **without** `--apply`, then every
...
- [ ] 14. Given a domain in `domains.yaml`, when its workspace is published, then the workspace
...
- [ ] 15. Given a grep of `src/globalmart/**/*.py`, when searching for any of the 12 domain keys or
...
- [ ] 16. Given a hand-edited `generated/workspaces/globalmart-sales.json`, when
...
- [ ] 17. Given a manifest whose `shared:` block names dashboards, visualizations and `shared.ai`
...
- [ ] 18. Given a domain declaring `ldm_include: [dim_store, dim_geo]` in `domains.yaml`, when its
